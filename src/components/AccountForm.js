...
class AccountForm extends React.Component {
  ...
  handleOnSubmit = (event) => {
    ...
    if (!allFieldsEntered) {
      ...
      if (selectedType === 'withdraw' && numericAmount <= total_balance) {
        this.props.dispatch(initiateWithdrawAmount(account.account_id, numericAmount));
        this.setState({ errorMsg: '' });
      } else if (selectedType === 'deposit') {
        this.props.dispatch(initiateDepositAmount(account.account_id, numericAmount));
        this.setState({ errorMsg: '' });
      } else {
        this.setState({
          errorMsg: {
    ...
    return account_no ? (
      editAccount ? (
        <div className="edit-account-form col-md-6 offset-md-3">
          <h3>
            Account details
            <a href="/#" className="edit-account" onClick={this.handleEditAccount}>
              Go Back
            </a>
          </h3>
          <hr />
        ...
        <div className="account-form col-md-6 offset-md-3">
          ...
          <Form onSubmit={this.handleOnSubmit} className="account-form">
            ...
            <Form.Group controlId="available_balance">
              <Form.Label>Available balance:</Form.Label>
              <span className="label-value">{account && account.total_balance}</span>
            </Form.Group>
